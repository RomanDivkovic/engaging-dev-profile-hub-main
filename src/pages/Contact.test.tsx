...
describe('Contact form', () => {
  ...
  it('shows validation errors for empty fields', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /send message/i }))
    expect(await screen.findAllByText(/must be at least|enter a valid email/i)).not.toHaveLength(0)
  })

  it('shows error toast on send failure', async () => {
    const emailjs = jest.requireMock('@emailjs/browser')
    emailjs.send.mockImplementationOnce(() => Promise.reject(new Error('fail')))
    render(
      <>
        <Toaster />
        <Contact />
